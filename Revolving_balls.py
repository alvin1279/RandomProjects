...
class Ball:
    ...
    def __init__(self,radius = 1,speed = 1,cl = True,trace= False):
        ...
        if rmax<self.r:
            rmax = self.r
            ax.set(xlim=(-(rmax + int(rmax*0.1)), (rmax + int(rmax*0.1))), ylim=(-(rmax + int(rmax*0.1)), (rmax + int(rmax*0.1))))
...
ball1 = Ball(10,trace=True,speed=10)
ball2 = Ball(15,speed=10,trace=True)
ball3 = Ball(30)
def animate(_):
    a = ball1.plot()
    b = ball2.plot()
